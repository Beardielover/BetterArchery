...
import zaggy1024.quivermod.client.renderers.Tex2DRender3D;
...
public class QuiverBowRenderer implements IItemRenderer {
	...
	public QuiverBowRenderer()
	{
		renderItem.setRenderManager(RenderManager.instance);
		
		try
		{
			improvedFirstPerson = ModImprovedFirstPerson.instance != null;
		}
		catch (LinkageError e)
		{
			improvedFirstPerson = false;
		}
	}
	...
	private void drawItem(Icon icon, float thickness)
	{
	...
	public void renderItem(ItemRenderType type, ItemStack bowStack, Object... data)
	{
		...
		if (type.equals(ItemRenderType.INVENTORY))
		{
            ...
            {
                Icon icon = item.getIcon(bowStack, pass);
		...
		else if (type.equals(ItemRenderType.ENTITY))
		{
            ...
            byte miniBlocks = renderItem.getMiniBlockCount(bowStack);
		...
		else if (type.equals(ItemRenderType.EQUIPPED) || type.equals(ItemRenderType.EQUIPPED_FIRST_PERSON))
		{
			...
			boolean wholeBow = !(bow instanceof BrokenBow);
			
			float partialTick = QuiverMod.proxy.overlayHandler.partialTick;
			GL11.glPushMatrix();
			...
			if (entity instanceof EntityPlayer)
			{
				...
				arrowStack = QuiverMod.playerValueManager.getUsingArrowStack(player.getEntityName());
			}
			else
			{
			...
			if (arrowStack == null)
				arrowStack = new ItemStack(Item.arrow);
			...
			if (wholeBow)
			{
				...
				if (iconOffset > 0)
				{
					...
					{
						icon = bow.getArrowIcon(iconOffset, arrowStack, pass);
