...
import zaggy1024.quivermod.client.quiverslots.SlotArrow;
import zaggy1024.quivermod.client.quiverslots.SlotReadOnly;
...
public class QuiverContainer extends Container {
	...
	public QuiverContainer(InventoryPlayer playerInv, QuiverInventory quiverInv) {
		...
		for (int i = 0; i < quiverInv.size; i++) {
			addSlotToContainer(new SlotArrow(quiverInv, i, x, 16));
			x += 18;
		}
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            else
            {
            	...
            	while (itemsLeft > 0 && i < quiverInventory.size)
            	{
            		...
            		if (stackInSlot != null && stackInSlot.stackSize < quiverInventory.stackLimit && stackInSlot.itemID == stack.itemID && stackInSlot.getItemDamage() == stack.getItemDamage())
            		{
    			...
    			quiverInventory.onInventoryChanged();
