...
public class QuiverModOverlayRenderer extends Gui {
	...
	public void pre(RenderGameOverlayEvent.Pre event)
	{
		if (GuiIngameForge.renderHotbar ? event.type == ElementType.HOTBAR : event.type == ElementType.ALL)
		{
			...
			long timeNow = QuiverMod.proxy.mc.getSystemTime();
			...
			if (mc != null)
			{
				...
				if (player != null)
				{
					...
					ArrayList<ItemStack> quiverStacks = valMan.getQuivers(player);
					...
					int selectedArrowItemIndex = valMan.getSelectedArrowItem(playerName);
					ArrayList<ItemStack> arrowTypeList = valMan.getArrowTypesHeld(player);
					...
					else if (selectedArrowItemIndex != 0)
					{
						...
						valMan.setSelectedArrowItem(playerName, selectedArrowItemIndex);
					}
					
					if (quiverCount > 0 && player.inventory.hasItem(QuiverMod.quiver.itemID))
					{
						...
						GL11.glTranslatef(0, QuiverModConfigManager.quiverHotbarVerticalOffset, 0);
						...
						if (QuiverModConfigManager.animationStyle == 0 && selectedQuiverSlots.size() > 0)
						{
							animTick += frameDiff;
						...
						for (ItemStack quiverStack : quiverStacks)
						{
							...
							int color = QuiverMod.quiver.getColorFromItemStack(quiverStack, 1);
							...
							for (int i = 0; i < quiverInventory.size; i++)
							{
								...
								if (hasArrowStack)
								{
									...
									else if (selected)
									{
										...
										if (QuiverModConfigManager.animationStyle == 1 && switchAnimation > 0)
										{
