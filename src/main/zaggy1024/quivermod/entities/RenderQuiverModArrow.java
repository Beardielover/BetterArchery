...
public class RenderQuiverModArrow extends Render
{
	...
	public static final String arrowTexturesPath = "quivermod:textures/arrows/";
    ...
    {
    	EntityQuiverModArrow modArrow = null;
    	
    	if (arrow instanceof EntityQuiverModArrow)
    		modArrow = (EntityQuiverModArrow)arrow;
    	...
    	if (distance3DSqr == -1)
    	{
    		distance3DSqr = QuiverModConfigManager.fancyArrowDistance * QuiverModConfigManager.fancyArrowDistance;
    	}
    	
    	boolean fancy = QuiverMod.proxy.mc.gameSettings.fancyGraphics && distanceSqr < distance3DSqr;
        ...
        if (modArrow.explosionSize > 0)
        {
        	renderArrowWithTex(impactArrowLoc, fancy);
        	
        	if (modArrow.explosionTime > 0)
        	{
        ...
        if (modArrow.teleportShooter)
        {
        	renderArrowWithTex(enderArrowLoc, fancy);
        }
        
        if (modArrow.placeTorch)
        {
        ...
        if (modArrow.potionDamage != -1)
        {
        	...
        	int color = QuiverMod.potionArrow.getColorFromDamage(modArrow.potionDamage);
			ColorHelper.glSetColor(color);
			
			if (modArrow.splashPotion)
			{
