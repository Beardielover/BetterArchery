...
import zaggy1024.quivermod.ModExplosion;
...
import zaggy1024.quivermod.util.PacketHelper;
import zaggy1024.quivermod.util.QuiverModMath;
...
public class EntityQuiverModArrow extends EntityArrow implements IProjectile, IEntityAdditionalSpawnData
{
    private float gravity = -0.05F;
	
	public int slowMo = QuiverModConfigManager.slowMoSetting;
	...
	public static class ImpaledItem {
		...
		public ImpaledItem(World world, int itemID, int damage, float position, float targetPosition)
		{
			item = createEntityItem(world);
    ...
    public EntityQuiverModArrow(World world, EntityLivingBase shooter, EntityLivingBase target, float speed, float deviation)
    {
        super(world);
        ...
        {
	        distance = MathHelper.sqrt_double(vecX * vecX + vecY * vecY + vecZ * vecZ);
    ...
    public EntityQuiverModArrow(World world, EntityLivingBase shooter, float speed)
    {
        super(world);
        ...
        /*Vec3 startPos = world.getWorldVec3Pool().getVecFromPool(shooter.posX, shooter.posY + shooter.getEyeHeight(), shooter.posZ);
        Vec3 endPos = world.getWorldVec3Pool().getVecFromPool(startPos.xCoord + motionX * 100, shooter.posY + shooter.getEyeHeight() + motionY * 100, shooter.posZ + motionZ * 100);
        MovingObjectPosition hit = world.rayTraceBlocks_do_do(startPos, endPos, false, true);

        if (hit != null)
        {
	        motionX = hit.hitVec.xCoord - posX;
	        motionY = hit.hitVec.yCoord - posY;
	        motionZ = hit.hitVec.zCoord - posZ;
    ...
    public void setThrowableHeading(double vecX, double vecY, double vecZ, float speed, float deviation)
    {
        ...
        vecX += rand.nextGaussian() * (double)(rand.nextBoolean() ? -1 : 1) * 0.007499999832361937D * (double)deviation;
        vecY += rand.nextGaussian() * (double)(rand.nextBoolean() ? -1 : 1) * 0.007499999832361937D * (double)deviation;
        vecZ += rand.nextGaussian() * (double)(rand.nextBoolean() ? -1 : 1) * 0.007499999832361937D * (double)deviation;
        vecX *= (double)speed;
        vecY *= (double)speed;
        vecZ *= (double)speed;
        motionX = vecX;
        ...
        prevRotationPitch = rotationPitch = (float)(Math.atan2(vecY, (double)horizDistance) * 180.0D / Math.PI);
        ticksInGround = 0;
    }
    ...
    protected void setBounds()
    {
    	if (worldObj != null)
    	{
	        ...
	        Vec3 headCoords = getRotatedCoords(posX, posY, posZ, 0.15F * arrowLength);
	        Vec3 tailCoords;
	        
	        if (inGround)
	        {
	        	tailCoords = getRotatedCoords(posX, posY, posZ, -0.6F * arrowLength);
	        }
	        else
	        {
	        	tailCoords = getRotatedCoords(posX - motionX, posY - motionY, posZ - motionZ, -0.6F * arrowLength);
	        }
    ...
    public void setValuesForStack(ItemStack arrowStack)
    {
        ...
        if (arrowStack != null)
        	...
        	drop = new ItemStack(QuiverMod.arrow);
        
        drop.stackSize = 1;
        
        if (arrowStack.itemID == QuiverMod.torchArrow.itemID)
        	drop = new ItemStack(Item.arrow);

        if (arrowStack.itemID == QuiverMod.fireArrow.itemID)
        {
            setFire(100);
        }
        
        if (arrowStack.itemID == QuiverMod.impactExplosiveArrow.itemID)
        {
        	setExplosive(1.5F);
        	setExplodeWhileInWater(true);
        }
        else if (arrowStack.itemID == QuiverMod.timedExplosiveArrow.itemID)
        {
        	setExplosive(3);
        	setExplosionTimer(40);
        }
        else if (arrowStack.itemID == QuiverMod.enderArrow.itemID)
        {
        	setTeleportShooter(true);
        }
        else if (arrowStack.itemID == QuiverMod.torchArrow.itemID)
        {
        	setPlaceTorch(true);
        }
        else if (arrowStack.itemID == QuiverMod.drillArrow.itemID)
        {
        	if (QuiverMod.drillArrow.isBroken(arrowStack.getItemDamage()))
        	{
        		setBreakBlocks(0.5F);
        	}
        	else
        	{
        		setBreakBlocks(6.5F);
        		drop.setItemDamage(QuiverMod.drillArrow.setBroken(drop.getItemDamage(), true));
        	}
        }
        else if (arrowStack.itemID == QuiverMod.potionArrow.itemID)
        {
        	ArrayList<PotionEffect> effects = (ArrayList<PotionEffect>)QuiverMod.potionArrow.getEffects(arrowStack);
        	setPotionEffects(arrowStack.getItemDamage(), effects);
        }

        if (arrowStack.getItem() instanceof ISplittingArrow)
        {
        	int splitCount = ((ISplittingArrow)arrowStack.getItem()).getSplittingArrowCount(arrowStack.getItemDamage());
        	...
        	if (splitCount > 1)
        	{
            	double mult = Math.pow(1.1, splitCount);
            	setSplit(2, splitCount, mult);
	...
	public void setPotionEffects(int damage, ArrayList<PotionEffect> effects)
	{
		...
		splashPotion = QuiverMod.potionArrow.isSplash(damage);
	}
	...
	private boolean tryExplodeNoDeath(double x, double y, double z)
	{
		if (explosionSize > 0 && !exploded)
		{
			...
			if (explodeInWater || (id != Block.waterStill.blockID && id != Block.waterMoving.blockID))
			{
				...
				{
					ModExplosion explosion = new ModExplosion(worldObj, this, x, y, z, explosionSize);
	...
	{
		if (teleportShooter && shootingEntity != null && shootingEntity instanceof EntityLivingBase)
		{
	...
	private void split()
	{
		if (lastHitMOP == null)
		{
	    	...
	    	if (pickupItem instanceof ISplittingArrow)
	    	{
	    		...
	    		pickupItemStack.setItemDamage(splittingPickupItem.getItemDamageForArrowCount(pickupItemStack.getItemDamage(), 1));
	    	}
	    	else
	    	{
	    	...
	    	if (!worldObj.isRemote)
	    	{
				...
				{
					EntityQuiverModArrow arrow = new EntityQuiverModArrow(worldObj);
					...
					arrow.setLocationAndAngles(prevPosX, prevPosY, prevPosZ, rotationYaw, rotationPitch);
	    	...
	    	/*if (worldObj.isRemote)
	    	{
	            ...
	            //worldObj.playSoundAtEntity(this, web.stepSound.getBreakSound(), (web.stepSound.getVolume() + 1.0F) / 2.0F, web.stepSound.getPitch() * 0.8F);
	            
	            for (int i = 0; i < 16; i++)
	            {
	            	QuiverMod.proxy.particleSpawning.spawnDiggingFX(worldObj, prevPosX, prevPosY, prevPosZ,
	            			0, 0, 0, web, 2, 0);
	            }
	    	}*/
	...
	private void onGroundImpact(MovingObjectPosition mop)
	{
		...
		if (placeTorch)
		{
			...
			if (!worldObj.isRemote)
				QuiverMod.tileEntityTorchBlock.tryPlace(worldObj, intX, intY, intZ, mop.hitVec.xCoord, mop.hitVec.yCoord, mop.hitVec.zCoord, mop.sideHit);
	...
	private void onDislodgedFromBlock(boolean broken)
	{
		...
		else
		{
	        ...
	        double motionDist = Math.sqrt(motionX * motionX + motionY * motionY + motionZ * motionZ);
	        motionX /= motionDist;
	        motionY /= motionDist;
	        motionZ /= motionDist;
			motionX *= 0.05;
	...
	private Vec3 getHitPos(MovingObjectPosition mop)
	{
    	...
    	return worldObj.getWorldVec3Pool().getVecFromPool(mop.hitVec.xCoord - offsetX, mop.hitVec.yCoord - offsetY, mop.hitVec.zCoord - offsetZ);
	}
	
	private boolean movePiston()
	{
		TileEntityPiston piston = null;
		Block inBlock = Block.blocksList[worldObj.getBlockId(xTile, yTile, zTile)];
		...
		if (inBlock == null)
		{
			orientation = BlockPistonBase.getOrientation(inData);
			...
			baseY += offY;
			baseZ += offZ;
			TileEntity tileEntity = worldObj.getBlockTileEntity(xTile + offX, yTile + offY, zTile + offZ);
		...
		{
			TileEntity tileEntity = worldObj.getBlockTileEntity(xTile, yTile, zTile);
		...
		if (piston != null)
		{
            ...
            {
                case 0:
                	pistonBounds = AxisAlignedBB.getAABBPool().getAABB(0, 0.25, 0, 1, 1, 1);
                	pistonBounds = AxisAlignedBB.getAABBPool().getAABB(0, 0, 0, 1, 0.25, 1);
                    break;
                case 1:
                	pistonBounds = AxisAlignedBB.getAABBPool().getAABB(0, 0, 0, 1, 0.75, 1);
                	pistonBounds = AxisAlignedBB.getAABBPool().getAABB(0, 0.75, 0, 1, 1, 1);
                    break;
                case 2:
                	pistonBounds = AxisAlignedBB.getAABBPool().getAABB(0, 0, 0.25, 1, 1, 1);
                	pistonBounds = AxisAlignedBB.getAABBPool().getAABB(0, 0, 0, 1, 1, 0.25);
                    break;
                case 3:
                	pistonBounds = AxisAlignedBB.getAABBPool().getAABB(0, 0, 0, 1, 1, 0.75);
                	pistonBounds = AxisAlignedBB.getAABBPool().getAABB(0, 0, 0.75, 1, 1, 1);
                    break;
                case 4:
                	pistonBounds = AxisAlignedBB.getAABBPool().getAABB(0.25, 0, 0, 1, 1, 1);
                	pistonBounds = AxisAlignedBB.getAABBPool().getAABB(0, 0, 0, 0.25, 1, 1);
                    break;
                case 5:
                	pistonBounds = AxisAlignedBB.getAABBPool().getAABB(0, 0, 0, 0.75, 1, 1);
                	pistonBounds = AxisAlignedBB.getAABBPool().getAABB(0.75, 0, 0, 1, 1, 1);
                	break;
            }
			...
			float lastProgress = piston.getProgress(0);
            ...
            /*pistonBounds = pistonBounds.addCoord(dirX * progress + dirX * expand,
            		dirY * progress + dirY * expand,
            		dirZ * progress + dirZ * expand);*/
            /*pistonBounds = pistonBounds.addCoord(dirX * (1 + expand),
            		dirY * (1 + expand),
            		dirZ * (1 + expand));*/
            pistonBounds.offset(dirX * progress,
            		dirY * progress,
            		dirZ * progress);
            pistonBounds = pistonBounds.addCoord(dirX * diff,
            		dirY * diff,
            		dirZ * diff);
            
            pistonBounds = pistonBounds.addCoord(dirX * expand,
            		dirY * expand,
            		dirZ * expand);
            pistonBounds = pistonBounds.expand(dirX == 0 ? expand : 0, dirY == 0 ? expand : 0, dirZ == 0 ? expand : 0);
			...
			if (pistonBounds.isVecInside(worldObj.getWorldVec3Pool().getVecFromPool(posX, posY, posZ)))
			{
				...
				if (!worldObj.isRemote)
				{
					progress = piston.getProgress(1);
					diff = progress - prevPistonProgress;
					prevPistonProgress = progress;
					setPosition(posX + offX * diff, posY + offY * diff, posZ + offZ * diff);
					forceTrackingUpdate();
				}
	...
	public void moveEntity(double x, double y, double z)
	{
		...
		if (allow)
		{
			...
			if (calledFrom.getClassName().equals("net.minecraft.tileentity.TileEntityPiston"))
			{
				allow = false;
			}
		}
	...
	public void forceTrackingUpdate()
	{
		if (!worldObj.isRemote)
		{
			...
			Set trackerSet = ObfuscationReflectionHelper.getPrivateValue(EntityTracker.class, tracker, "trackedEntities", "field_72793_b");
	...
	public void sendImpaledItemsPacket()
	{
		...
		PacketDispatcher.sendPacketToAllInDimension(PacketHelper.makePacket("QMArImpaledItems", dataList.toArray()), dimension);
	}
	
	public void checkItemsToImpale(Vec3 fromVec, Vec3 toVec)
	{
		//Vec3 fromVec = worldObj.getWorldVec3Pool().getVecFromPool(posX, posY, posZ);
		//Vec3 toVec = worldObj.getWorldVec3Pool().getVecFromPool(posX + motionX, posY + motionY, posZ + motionZ);
        
        //Vec3 fromVec = worldObj.getWorldVec3Pool().getVecFromPool(prevX, prevY, prevZ);
        //Vec3 toVec = worldObj.getWorldVec3Pool().getVecFromPool(posX, posY, posZ);
        
		if (worldObj.isRemote)
			return;
		
        AxisAlignedBB checkBounds = boundingBox
        		.addCoord(motionX, motionY, motionZ)
        		.expand(1, 1, 1);
        List<Entity> entityList = worldObj.getEntitiesWithinAABBExcludingEntity(this, checkBounds);
        ...
        for (Entity entity : entityList)
        {
        	if (entity instanceof EntityItem)
        	{
        		...
        		if (stack != null)
        		{
	        		...
	        		boolean full3D = ObfuscationReflectionHelper.getPrivateValue(Item.class, item,
	        				"bFull3D", "field_77789_bW");
	        		...
	        		{
		                AxisAlignedBB bobbedBB = itemEnt.boundingBox.copy().offset(0, 0.05F, 0).addCoord(0, 0.3F, 0);
		                this.setRenderBB(bobbedBB);
		                
		                MovingObjectPosition hit = bobbedBB.calculateIntercept(fromVec, toVec);
		                
		                if (hit != null)
		                {
                			hitItems.put(fromVec.distanceTo(hit.hitVec), itemEnt);
		                }
	        		}
        		}
        	}
    	...
    	if (slowMo < 1 || actualTicks % slowMo == 0)
    	{
            ...
            if (lastHitMOP != null)
            {
                ...
                if (lastHitMOP.typeOfHit == EnumMovingObjectType.ENTITY)
                {
                	if (lastHitMOP.entityHit.isEntityAlive())
                	{
                		...
                		if (lastHitMOP.entityHit instanceof EntityLivingBase)
                		{
                			prevHealth = ((EntityLivingBase)lastHitMOP.entityHit).getHealth();
                		}
	                    ...
	                    if (explosionTime < 0 && lastHitMOP.entityHit.attackEntityFrom(damageSource, hitDamage))
	                    {
	                        if (lastHitMOP.entityHit instanceof EntityLivingBase)
	                        {
	                            ...
	                            float damageTaken = prevHealth - entityLiving.getHealth();
	                            ...
	                            {
	                                horizMotion = MathHelper.sqrt_double(motionX * motionX + motionZ * motionZ);
	                                
	                                if (horizMotion > 0.0F)
	                                {
	                                	lastHitMOP.entityHit.addVelocity(motionX * (double)knockbackStrength * 0.6 / (double)horizMotion, 0.1, motionZ * (double)knockbackStrength * 0.6 / (double)horizMotion);
	                                }
	                            }

	                            double motionDiffX = lastHitMOP.entityHit.motionX - lastEntityMotionX;
	                            double motionDiffY = lastHitMOP.entityHit.motionY - lastEntityMotionY;
	                            double motionDiffZ = lastHitMOP.entityHit.motionZ - lastEntityMotionZ;
	                            float motionMult = 1;

	                            if (knockbackMode > 0)
	                            {
	                            	float armor = (1 - damageTaken / (float)hitDamage) * 20;
		                            ...
		                            case 1:
	                                	...
	                                	break;
		                            case 2:
	                                	motionMult /= Math.max(1, (20 - armor) / 4);
	                                	break;
		                            }
	                            }
                                ...
                                lastHitMOP.entityHit.motionX = lastEntityMotionX + motionDiffX * motionMult;
                                lastHitMOP.entityHit.motionY = lastEntityMotionY + motionDiffY * motionMult;
                                lastHitMOP.entityHit.motionZ = lastEntityMotionZ + motionDiffZ * motionMult;
	                		...
	                		explode(lastHitMOP.hitVec.xCoord, lastHitMOP.hitVec.yCoord, lastHitMOP.hitVec.zCoord);
	                		
	                		if (breakBlocks < 1 && !(lastHitMOP.entityHit instanceof EntityEnderman))
	                		{
	                    ...
	                    else if (lastHitMOP.entityHit != shootingEntity || ticksInAir >= 5)
	                    {
                ...
                else	// TILE
                {
                    ...
                    if (inTile > 0)
                    {
	                    ...
	                    if (block != null)
	                    {
	                    	...
	                    	if (stepSound != null)
	                    		playSound(stepSound.getPlaceSound(), volume, 1.2F / (rand.nextFloat() * 0.2F + 0.9F));
	                    }
	        ...
	        if (inGround)
	        {
	            ...
	            if (prevInBlock != null)
	            {
	            	...
	            	{
	            		associated = inNowBlock.idDropped(metadata, rand, 0) == prevInBlock.idDropped(inData, rand, 0);
	            	}
	            
	            	if (pistonMoved)
	            	{
		            	if (inNowBlock == null || inNowBlock instanceof BlockPistonMoving || inNowBlock instanceof BlockPistonExtension || inNowBlock instanceof BlockPistonBase)
		            	{
		            		associated = true;
	            	...
	            	else if (inData != metadata && inNowBlock != null && prevInBlock instanceof BlockPistonBase && (inNowBlock instanceof BlockPistonBase || inNowBlock instanceof BlockPistonMoving))
	            	{
	            		associated = true;
	            		pistonMoved = true;
	            		movePiston();
	            	}
	            	else if (inTile != blockID && prevInBlock instanceof BlockPistonExtension && inNowBlock == null)
	            	{
	            		associated = true;
	            	...
	            	{
	            		associated = prevInBlock instanceof BlockPistonMoving && (inNowBlock instanceof BlockPistonExtension || inNowBlock instanceof BlockPistonBase);
	            	}
	
	            	if (associated)
	            	{
	    	            blockID = worldObj.getBlockId(xTile, yTile, zTile);
	    	            metadata = worldObj.getBlockMetadata(xTile, yTile, zTile);
	            ...
	            if (associated)
	            {
	            	if (inNowBlock != null && (inData != metadata || inNowBlock instanceof BlockDoor))
	            	{
	            		MovingObjectPosition mop = null;
	            		
	            		if (inNowBlock.canCollideCheck(metadata, false) && inNowBlock.getCollisionBoundingBoxFromPool(worldObj, xTile, yTile, zTile) != null)
	            		{
			            	double motion = Math.sqrt(motionX * motionX + motionY * motionY + motionZ * motionZ);
			            	...
			            	Vec3 pos1 = worldObj.getWorldVec3Pool().getVecFromPool(posX - normMotionX * 0.1,
			            			posY - normMotionY * 0.1,
			            			posZ - normMotionZ * 0.1);
			            	Vec3 pos2 = worldObj.getWorldVec3Pool().getVecFromPool(posX + normMotionX,
			            			posY + normMotionY,
			            			posZ + normMotionZ);
			            	
			            	mop = inNowBlock.collisionRayTrace(worldObj, xTile, yTile, zTile, pos1, pos2);
	            		}
	    	            ...
	    	            if (mop != null && !pistonMoved)
	    	            {
	    	            	AxisAlignedBB smallBounds = AxisAlignedBB.getBoundingBox(posX, posY, posZ, posX, posY, posZ).expand(0.08, 0.08, 0.08);
                	...
                	/*if (pistonMoved && !(worldObj.getBlockTileEntity(xTile, yTile, zTile) instanceof TileEntityPiston))
                	{
                		QuiverMod.log("resetmoved");
    	            	pistonMoved = false;
                	}*/
	            }
	            else
	        ...
	        else
	        {
	        	...
	        	while (fromVec == null || mop != null)
	        	{
		            ...
		            if (!worldObj.isRemote && mop != null)
		            {
		            	...
		            	if (hardness > 0)
		            	{
			            	...
			            	float toolHardness = 6 - Item.pickaxeIron.getStrVsBlock(new ItemStack(Item.pickaxeIron), block, metadata);
			            	...
			            	if (breakBlocks >= damage)
			            	{
			            		...
			            		worldObj.playAuxSFX(2001, mop.blockX, mop.blockY, mop.blockZ, id);
			                    block.dropBlockAsItemWithChance(worldObj, mop.blockX, mop.blockY, mop.blockZ, metadata, 0.8F, 0);
	            ...
	            List entityList = worldObj.getEntitiesWithinAABBExcludingEntity(this, boundingBox.addCoord(motionX, motionY, motionZ).expand(1.0D, 1.0D, 1.0D));
	            ...
	            for (int i = 0; i < entityList.size(); ++i)
	            {
	                ...
	                if (curEntity.canBeCollidedWith() && (ticksInAir >= 5 || curEntity != shootingEntity))
	                {
	                	...
	                	if (rideEntity == null || (rideEntity.canBeCollidedWith() && (ticksInAir >= 5 || curEntity != rideEntity)))
	                	{
	            ...
	            if (mop != null && (mop.typeOfHit == EnumMovingObjectType.ENTITY ? canAttackEntity(mop.entityHit) : true))
	            {
                	...
                	{
                		Vec3 hitPos = getHitPos(mop);
                		setPosition(hitPos.xCoord - motionX, hitPos.yCoord - motionY, hitPos.zCoord - motionZ);
	            ...
	            for (targetPitch = (float)(Math.atan2(motionY, (double)horizMotion) * 180.0D / Math.PI); targetPitch - prevRotationPitch < -180.0F; prevRotationPitch -= 360.0F)
	            {
	                ;
	            }
	            ...
	            float pitchDiff = QuiverModMath.clamp(rotationPitch - targetPitch, -maxRotSpeed, maxRotSpeed);
	            ...
	            if (horizMotion > 0.1F)
	            {
		            ...
		            float yawDiff = QuiverModMath.clamp(rotationYaw - targetYaw, -maxRotSpeed, maxRotSpeed);
		            
		            //rotationYaw = prevRotationYaw + (targetYaw - prevRotationYaw) * 0.2F;
	            ...
	            if (this.isInWater())
	            {
	                ...
	                {
	                    horizMotion = 0.25F;
	                    this.worldObj.spawnParticle("bubble", this.posX - this.motionX * (double)horizMotion, this.posY - this.motionY * (double)horizMotion, this.posZ - this.motionZ * (double)horizMotion, this.motionX, this.motionY, this.motionZ);
	                }
			...
			if ((explosionTime > 0 && ticks >= explosionTime) || blockID == Block.lavaStill.blockID || blockID == Block.lavaMoving.blockID)
			{
			...
			if (breakBlocks > 0 && !inGround)
			{
				...
				worldObj.playSoundAtEntity(this, pistonSound, 0.3F, worldObj.rand.nextFloat() * 0.25F + 0.6F);
			}
	...
	{
        if ((potionEffects != null && !potionEffects.isEmpty()) || potionDamage != -1)
        {
        	if (worldObj.isRemote)
        	{
                ...
                for (int i = 0; i < 8; ++i)
                {
                	renderGlobal.spawnParticle(particle, posX, posY, posZ, worldObj.rand.nextGaussian() * 0.15D, worldObj.rand.nextDouble() * 0.2D, worldObj.rand.nextGaussian() * 0.15D);
                }
                ...
                for (int i = 0; i < 100; ++i)
                {
                    ...
                    EntityFX particleEntity = renderGlobal.doSpawnParticle(particleName, posX + velX * 0.1D, posY + 0.3D, posZ + velZ * 0.1D, velX, velY, velZ);

                    if (particleEntity != null)
                    {
                        float brightness = 0.75F + worldObj.rand.nextFloat() * 0.25F;
                        particleEntity.setRBGColorF(red * brightness, green * brightness, blue * brightness);
                        particleEntity.multiplyVelocity((float)distance);
                    }
                }

                worldObj.playSound(posX, posY, posZ, "random.glass", 1, worldObj.rand.nextFloat() * 0.1F + 0.9F, false);
        	}
        	else
        	{
	            AxisAlignedBB axisalignedbb = this.boundingBox.expand(4.0D, 2.0D, 4.0D);
	            List<EntityLivingBase> entityList = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, axisalignedbb);
	
	            if (entityList != null && !entityList.isEmpty())
	            {
	                Iterator<EntityLivingBase> entityIter = entityList.iterator();
	                ...
	                while (entityIter.hasNext())
	                {
	                    ...
	                    if (distSqr < 16.0D)
	                    {
	                        ...
	                        {
	                            PotionEffect potioneffect = (PotionEffect)effectsIter.next();
	                            int potionID = potioneffect.getPotionID();
	
	                            if (Potion.potionTypes[potionID].isInstant())
	                            {
	                                Potion.potionTypes[potionID].affectEntity((EntityLivingBase)shootingEntity, checkingEntity, potioneffect.getAmplifier(), power);
	                            }
	                            else
	                            {
	                                int duration = (int)(power * (double)potioneffect.getDuration() + 0.5D);
	
	                                if (duration > 20)
	                                {
	                                    checkingEntity.addPotionEffect(new PotionEffect(potionID, duration, potioneffect.getAmplifier()));
	                                }
	                            }
	                        }
	                    }
	...
	public void readSpawnData(ByteArrayDataInput data) {
		...
		setLocationAndAngles(posX, posY, posZ, data.readFloat(), data.readFloat());
        ...
        if (worldObj.isRemote)
        {
        	PacketDispatcher.sendPacketToServer(PacketHelper.makePacket("QMArGetSpecial", entityId));
        }
	}
    ...
    public void writeEntityToNBT(NBTTagCompound tagCompound)
    {
    	...
    	if (potionEffects != null && !potionEffects.isEmpty())
    	{
    		...
    		NBTTagList effectsList = tagCompound.getTagList("CustomPotionEffects");
    		
    		for (PotionEffect effect : potionEffects)
    		{
    			effectsList.appendTag(effect.writeCustomPotionEffectToNBT(new NBTTagCompound()));
    		}
    	}
    ...
    public void readEntityFromNBT(NBTTagCompound tagCompound)
    {
        ...
        if (tagCompound.hasKey("pickupItemStack"))
        {
	    	...
	    	{
	    		ItemStack potionArrow = new ItemStack(QuiverMod.potionArrow.itemID, 1, tagCompound.getInteger("potionDamage"));
	    		
	    		if (potionArrow.stackTagCompound == null)
	    			potionArrow.stackTagCompound = new NBTTagCompound();
	    		
	    		potionArrow.stackTagCompound.setTag("CustomPotionEffects", tagCompound.getTagList("CustomPostionEffects"));
	    		
	    		setPotionEffects(potionArrow.getItemDamage(),
	    				(ArrayList<PotionEffect>)QuiverMod.potionArrow.getEffects(potionArrow));
	    	}
	    	...
	    	{
	    		NBTTagCompound itemComp = (NBTTagCompound)impaledItemList.tagAt(i);
    ...
    public void onCollideWithPlayer(EntityPlayer player)
    {
		if (!worldObj.isRemote && inGround && arrowShake <= 0)
		{
			...
			if (canBePickedUp == 1 && !player.inventory.addItemStackToInventory(pickupItemStack))
			{
				pickedUp = false;
			}
